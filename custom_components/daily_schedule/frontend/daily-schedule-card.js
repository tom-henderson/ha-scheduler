...
`;let Ct=class extends at{constructor(){super(...arguments),this._name="",this._targets=[],this._save=()=>{this.dispatchEvent(new CustomEvent("bar-settings-save",{detail:{name:this._name.trim()||this.bar.name,targets:this._targets}}))}}willUpdate(t){t.has("bar")&&(this._name=this.bar.name,this._targets=[...this.bar.targets])}render(){const t={entity:{multiple:!0,filter:{domain:ft[this.bar.type]??[]}}};return W`
    ...
    `],t([pt({attribute:!1})],Ct.prototype,"hass",void 0),t([pt({attribute:!1})],Ct.prototype,"bar",void 0),t([ut()],Ct.prototype,"_name",void 0),t([ut()],Ct.prototype,"_targets",void 0),Ct=t([lt("ds-bar-settings")],Ct);let zt=class extends at{constructor(){super(...arguments),this.scheduleOn=!0,this.conflicts=[],this.now=0,this.syncing=!1,this._editing=null,this._drag=null,this._moved=!1,this._addSegment=()=>{const t=function(t){const e=t.segments.map(t=>[t.start,t.end]).sort((t,e)=>t[0]-e[0]);let i=null,s=0;const n=t=>t?t[1]-t[0]:0;for(const[t,r]of e)t-s>n(i)&&(i=[s,t]),s=Math.max(s,r);if(vt-s>n(i)&&(i=[s,vt]),!i||n(i)<.5)return null;const r=i[0];return{start:r,end:Math.min(i[1],r+Math.max(1,n(i)/2))}}(this.bar);if(!t)return;const e=0===this.bar.base?1:0,i={id:`seg_${Math.random().toString(36).slice(2,10)}`,start:t.start,end:t.end,state:e,jitter:0};this._emit({...this.bar,segments:[...this.bar.segments,i]},!0),this._editing=i.id},this._saveSegment=t=>{const e=t.detail;this._emit({...this.bar,segments:this.bar.segments.map(t=>t.id===e.id?e:t)},!0),this._editing=null},this._deleteSegment=t=>{this._emit({...this.bar,segments:this.bar.segments.filter(e=>e.id!==t.detail)},!0),this._editing=null},this._pickBase=t=>{this._emit({...this.bar,base:t.detail},!0),this._editing=null},this._saveSettings=t=>{this._emit({...this.bar,name:t.detail.name,targets:t.detail.targets},!0),this._editing=null}}get _live(){return this.scheduleOn&&this.bar.enabled}_emit(t,e){this.dispatchEvent(new CustomEvent("bar-change",{detail:{bar:t,commit:e}}))}_dragSeg(t,e,i){i.stopPropagation(),this._moved=!1;const s=St(this.bar,t),n=i.clientX,r=t.start,o=t.end;this._drag={id:t.id,mode:e};const a=this.renderRoot.querySelector(".track"),d=i=>{const d=a.getBoundingClientRect().width,l=Math.round((i.clientX-n)/d*vt/bt)*bt;Math.abs(i.clientX-n)>3&&(this._moved=!0);let c={...t};if("move"===e){const e=o-r,i=Math.min(s.max-e,Math.max(s.min,r+l));c={...t,start:i,end:i+e}}else c="l"===e?{...t,start:Math.min(o-bt,Math.max(s.min,r+l))}:{...t,end:Math.max(r+bt,Math.min(s.max,o+l))};this._emit({...this.bar,segments:this.bar.segments.map(e=>e.id===t.id?c:e)},!1)},l=()=>{window.removeEventListener("pointermove",d),window.removeEventListener("pointerup",l),this._drag=null,this._moved&&this._emit(this.bar,!0)};window.addEventListener("pointermove",d),window.addEventListener("pointerup",l)}render(){const t=yt(this.bar.type),e=this.types[this.bar.type],i=Et(this.bar),s=wt(this.types,this.bar.type,this.bar.base),n=i.find(t=>t.id===this._editing),r=(o=this.bar,a=this.now,Et(o).find(t=>a>=t.start&&a<t.end));var o,a;return W`
      <div class="bar" style=${`opacity:${this._live?1:.4};--accent:${t}`}>
        ...
        <div class="track" @click=${()=>this._editing="__base__"}>
          ...
          ${this.syncing&&this._live&&r?W`<div
                class="flash"
                style=${`left:${r.start/vt*100}%;width:${(r.end-r.start)/vt*100}%`}
              ></div>`:V}
    ...
    `}_renderSegment(t){const e=wt(this.types,this.bar.type,t.state),i=t.start/vt*100,s=(t.end-t.start)/vt*100,n=At(this.types,this.bar.type,t.state),r=this._drag?.id===t.id,o=r&&("move"===this._drag.mode||"l"===this._drag.mode),a=r&&("move"===this._drag.mode||"r"===this._drag.mode);return W`
      <div
        class=${`seg ${e?"active":"inactive"} ${r?"dragging":""}`}
        style=${`left:${i}%;width:${s}%`}
        title=${`${$t(t.start)}–${$t(t.end)} · ${n}${t.jitter?` · ${function(t){return(mt.find(e=>Math.abs(e.v-t)<.001)??mt[0]).label}(t.jitter)}`:""}`}
      ...
      >
        ...
        ${s>8?W`<span class="seg-label"
              >${n}${t.jitter?W`<ha-icon icon="mdi:dice-5" style="--mdc-icon-size:12px"></ha-icon>`:V}</span
            >`:V}
        <div class="handle r" @pointerdown=${e=>this._dragSeg(t,"r",e)}>
          <span></span>
        </div>
        ${o?W`<div class="bubble l">${$t(t.start)}</div>`:V}
        ${a?W`<div class="bubble r">${$t(t.end)}</div>`:V}
      </div>
    `}};zt.styles=[kt,o`
    .bar {
      margin-bottom: 14px;
  ...
  `],t([pt({attribute:!1})],zt.prototype,"hass",void 0),t([pt({attribute:!1})],zt.prototype,"bar",void 0),t([pt({attribute:!1})],zt.prototype,"types",void 0),t([pt({type:Boolean})],zt.prototype,"scheduleOn",void 0),t([pt({attribute:!1})],zt.prototype,"conflicts",void 0),t([pt({type:Number})],zt.prototype,"now",void 0),t([pt({type:Boolean})],zt.prototype,"syncing",void 0),t([ut()],zt.prototype,"_editing",void 0),t([ut()],zt.prototype,"_drag",void 0),zt=t([lt("ds-bar")],zt);let Pt=class extends at{constructor(){super(...arguments),this.accent="",this._si=0,this._start="",this._end="",this._jit=0,this._err="",this._close=()=>this.dispatchEvent(new CustomEvent("popover-close")),this._delete=()=>this.dispatchEvent(new CustomEvent("segment-delete",{detail:this.segment.id}))}willUpdate(t){t.has("segment")&&(this._si=this.segment.state,this._start=$t(this.segment.start),this._end=$t(this.segment.end),this._jit=this.segment.jitter??0,this._err="")}_save(){const t=xt(this._start),e=xt(this._end);return null==t||null==e?this._fail("Use HH:MM"):e<=t?this._fail("End must be after start"):t<this.bounds.min||e>this.bounds.max?this._fail(`Stay within ${$t(this.bounds.min)}–${$t(this.bounds.max)}`):void this.dispatchEvent(new CustomEvent("segment-save",{detail:{...this.segment,state:this._si,start:t,end:e,jitter:this._jit}}))}_fail(t){this._err=t}render(){const t=this.types[this.type]?.states??[];return W`
      <div class="popover" style="width:268px" @click=${t=>t.stopPropagation()}>
        ...
        <div class="state-buttons" style=${`--accent:${this.accent}`}>
          ${t.map((t,e)=>W`
              <button class=${e===this._si?"sel":""} @click=${()=>this._si=e}>
                ${t.label}
              </button>
            `)}
        </div>
    ...
    `],t([pt({attribute:!1})],Pt.prototype,"segment",void 0),t([pt()],Pt.prototype,"type",void 0),t([pt({attribute:!1})],Pt.prototype,"types",void 0),t([pt({attribute:!1})],Pt.prototype,"bounds",void 0),t([pt({attribute:!1})],Pt.prototype,"accent",void 0),t([ut()],Pt.prototype,"_si",void 0),t([ut()],Pt.prototype,"_start",void 0),t([ut()],Pt.prototype,"_end",void 0),t([ut()],Pt.prototype,"_jit",void 0),t([ut()],Pt.prototype,"_err",void 0),Pt=t([lt("ds-segment-editor")],Pt);let Mt=class extends at{constructor(){super(...arguments),this.accent=""}render(){const t=this.types[this.bar.type]?.states??[];return W`
  ...
  `,t([pt({attribute:!1})],Ot.prototype,"hass",void 0),t([ut()],Ot.prototype,"_config",void 0),t([ut()],Ot.prototype,"_entries",void 0),Ot=t([lt("ds-editor")],Ot);const jt=()=>{const t=new Date;return t.getHours()+t.getMinutes()/60+t.getSeconds()/3600};let Ut=class extends at{constructor(){super(...arguments),this._types={},this._now=jt(),this._syncing=!1,this._menuOpen=!1,this._connecting=!1,this._onBarChange=(t,e)=>{this._schedule&&(this._schedule={...this._schedule,bars:this._schedule.bars.map(e=>e.id===t.id?t:e)},e&&this._ws("update_bar",{bar_id:t.id,bar:t}))}}static getConfigElement(){return document.createElement("ds-editor")}static getStubConfig(){return{entry_id:""}}setConfig(t){this._config=t,this._entryId=t.entry_id||void 0,this._connectedEntry&&this._connectedEntry!==this._entryId&&this._teardown()}connectedCallback(){super.connectedCallback(),this._nowTimer=window.setInterval(()=>this._now=jt(),3e4),this._maybeConnect()}disconnectedCallback(){super.disconnectedCallback(),this._nowTimer&&window.clearInterval(this._nowTimer),this._teardown()}updated(){this._maybeConnect()}async _maybeConnect(){if(this.hass&&!this._connectedEntry&&!this._connecting){this._connecting=!0;try{await this._connect()}finally{this._connecting=!1}}}async _connect(){if(!this._entryId)try{const t=await this.hass.callWS({type:`${gt}/list_entries`});if(this._types=t.types,1!==t.entries.length)return 0===t.entries.length?void(this._error="No Daily Schedule is set up. Add the integration first."):void(this._error="Multiple schedules found — set entry_id in the card config.");this._entryId=t.entries[0].entry_id}catch(t){return void(this._error=`Could not reach Daily Schedule: ${t}`)}this._connectedEntry=this._entryId,this._error=void 0;try{this._unsub=this.hass.connection.subscribeMessage(t=>{this._schedule=t},{type:`${gt}/subscribe`,entry_id:this._entryId});const t=await this.hass.callWS({type:`${gt}/get`,entry_id:this._entryId});this._types=t.types,this._schedule=t}catch(t){this._error=`Could not load schedule: ${t}`,this._connectedEntry=void 0}}_teardown(){this._unsub?.then(t=>t()).catch(()=>{}),this._unsub=void 0,this._connectedEntry=void 0,this._schedule=void 0}async _ws(t,e={}){try{await this.hass.callWS({type:`${gt}/${t}`,entry_id:this._entryId,...e})}catch(e){this._error=`${t} failed: ${e}`}}_setEnabled(t){this._schedule&&(this._schedule={...this._schedule,enabled:t}),this._ws("set_enabled",{enabled:t})}async _syncNow(){this._schedule?.enabled&&(this._syncing=!0,await this._ws("sync_now"),window.setTimeout(()=>this._syncing=!1,1400))}_addBar(t){this._menuOpen=!1,this._ws("add_bar",{bar:{name:"New schedule",type:t,targets:[],base:0,enabled:!0,segments:[{start:8,end:18,state:1,jitter:0}]}})}render(){if(this._error)return this._card(W`<div class="error">${this._error}</div>`);if(!this._schedule)return this._card(W`<div class="loading">Loading…</div>`);const t=this._schedule,e=t.bars.length,i=this._config?.title??"Daily Schedule";return this._card(W`
    ...
    `)}_card(t){return W`<ha-card>${t}</ha-card>`}getCardSize(){return 3+(this._schedule?.bars.length??1)}};Ut.styles=[kt,o`
    ha-card {
      overflow: visible;
    }
    .error,
  ...
  `],t([pt({attribute:!1})],Ut.prototype,"hass",void 0),t([ut()],Ut.prototype,"_config",void 0),t([ut()],Ut.prototype,"_schedule",void 0),t([ut()],Ut.prototype,"_types",void 0),t([ut()],Ut.prototype,"_entryId",void 0),t([ut()],Ut.prototype,"_error",void 0),t([ut()],Ut.prototype,"_now",void 0),t([ut()],Ut.prototype,"_syncing",void 0),t([ut()],Ut.prototype,"_menuOpen",void 0),Ut=t([lt("daily-schedule-card")],Ut),window.customCards=window.customCards||[],window.customCards.push({type:"daily-schedule-card",name:"Daily Schedule",description:"Build a repeating 24-hour schedule that drives your entities.",preview:!1});export{Ut as DailyScheduleCard};
